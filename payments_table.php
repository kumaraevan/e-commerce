...
if (!empty($search)) {
    $whereClause = "WHERE PaymentID LIKE CONCAT('%', ?, '%') 
...
$sql = "SELECT * FROM payment $whereClause ORDER BY PaymentID ASC";
...
<body class="bg-gray-100">
    ...
    <div class="container mx-auto px-4">
        <h2 class="text-2xl font-semibold my-4">Manage Payment</h2>

        <!-- Search bar -->
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="GET" class="mb-4">
            <div class="flex items-center">
                <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search payment" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
        ...
        <table class="table-auto w-full mb-4">
            ...
            <tbody>
                ...
                <tr class="bg-white">
                    ...
                    <td class="border px-4 py-2"><a href="edit_product.php?id=<?php echo $row["PaymentID"]; ?>" class="text-blue-500 hover:text-blue-800">Edit</a></td>
                    <td class="border px-4 py-2"><a href="delete_product.php?id=<?php echo $row["PaymentID"]; ?>" onclick="return confirm('Are you sure you want to delete this product?')" class="text-red-500 hover:text-red-800">Delete</a></td>
                </tr>
                <?php } ?>
            </tbody>
        </table>
